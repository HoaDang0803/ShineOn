...
class FirebaseDatabaseRepository @Inject constructor(
    ...
    fun addToCart(
        ...
        val cartReference = databaseReference.child("carts").child(user.uid)
        cartReference.child(product.id).setValue(product)
            .addOnSuccessListener {
                Log.d("FirebaseDatabaseRepository", "Product added to cart: ${product.title}")
                onSuccess()
            }
            .addOnFailureListener { exception ->
                Log.e("FirebaseDatabaseRepository", "Failed to add product: $exception")
                onError(exception)
            }
    }
